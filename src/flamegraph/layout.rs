...
pub(super) fn layout(tree: &DepTreeData) -> Vec<LayoutRect> {
    ...
    for &root_idx in &tree.root_indices {
        ...
        layout_node(tree, root_idx, x, 0, w, &mut state, "");
        x += w;
    }
...
fn layout_node(
    ...
    if width < MIN_RECT_WIDTH || depth > MAX_DEPTH || state.rects.len() >= MAX_FRAMES {
        return;
    }

    // Cycle detection on the current path only (not global).
    if !state.path.insert(node_idx) {
        return;
    }

    let node = &tree.nodes[node_idx];
    let is_shared = state.ancestor_counts[node_idx] > 1;

    let rect_idx = state.rects.len();
    state.rects.push(LayoutRect {
        x,
        ...
        ancestor_count: state.ancestor_counts[node_idx],
        collapsed_children: 0,
    });

    if node.children.is_empty() {
        state.path.remove(&node_idx);
        return;
    }
    ...
    if collapsed > 0 {
        state.rects[rect_idx].collapsed_children = collapsed;
    }
