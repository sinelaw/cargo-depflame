...
use crate::cargo_toml::CrateDepInfo;
...
pub fn run_analyze(args: &AnalyzeArgs) -> Result<AnalysisReport> {
    ...
    let targets: Vec<UpstreamTarget> = edges
        .par_iter()
        .filter_map(|edge| {
            scan_edge(edge, &dep_graph, &metadata, &cargo_toml_cache, &real_deps)
        })
        .collect();
...
/// Scan a single intermediate edge: locate source, parse Cargo.toml, run
/// heuristic scanner, and compute metrics.
fn scan_edge(
    edge: &IntermediateEdge,
    dep_graph: &DepGraph,
    metadata: &cargo_metadata::Metadata,
    cargo_toml_cache: &Mutex<HashMap<String, CrateDepInfo>>,
